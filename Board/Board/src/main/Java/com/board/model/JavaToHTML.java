...
public class JavaToHTML {
    ...
    private JavaToHTML() {}
    public static JavaToHTML getInstance() {
        if (instance == null) {
    ...
    public static String markIO(Object obj, String outtype, String forms) {
        ...
        if(forms.equals(table) || forms.equals(select)) {
            outString.append("<").append(forms).append(">").append(outString.toString()).append("</").append(forms).append(">");
        }
    ...
    private static void processHtml(Object subobj, String forms, StringBuilder outString) {
        if (forms.equals(table) || forms.equals(select)) {
            if (forms.equals(table)) {
                outString.append("<tr><td><a href=").append("<%request.getContextPath()%>/content?${subobj}").append("${subobj}>").append("${subobj}").append("</td>");
